<template>
    <div class="general-editor-container" data-testid="general-editor-container">
        <Card :title="cardProps.title" :subtitle="cardProps.subtitle" :text="cardProps.text">
            <template #content>
                <div v-if="model" class="general-editor-content" data-testid="general-editor-content">
                    <h2 class="properties-label">{{ propertiesLabel }} {{ generalProperties.$type }}</h2>
                    ...
                    <Select :label="inputLabel.taskType" v-model="selectedTaskType" :selectOptionItems="taskTypes" :selectItemClickHandler="updateTaskType" />
                    <Select :label="inputLabel.gatewayType" v-model="selectedGatewayType" :selectOptionItems="gatewayTypes" :selectItemClickHandler="updateGatewayType"/>
                </div>                
            </template>
        </Card>
    </div>
